...
public class SandboxToolsConfig {
    ...
    public void cleanExpiredData() {
        ...
        while (it2.hasNext()) {
             ...
             * 超过30分钟未访问的代码，删除
             */
            if (now - entry2.getValue().createdAt > expireMs) { it2.remove(); removed++; }
        }
        if (removed > 0) {
    ...
    public FunctionCallback executeCodeFunction(
        ...
        return FunctionCallback.builder()
                ...
                .function("execute_code", (Function<Map<String, Object>, Map<String, Object>>) params -> {
                    ...
                    log.info("[SandboxTool] 代码执行工具被调用, 执行Id={}, code length={}",
    ...
    public FunctionCallback fixCodeFunction(
        ...
        return FunctionCallback.builder()
                ...
                .function("fix_code", (Function<Map<String, Object>, Map<String, Object>>) params -> {
                    ...
                    Map<String, String> dataRefsMap = null;
                    ...
                    if (dataRefsObj != null) {
                        ...
                        try {
                            ...
                            log.info("[FixCode] 多数据集模式，加载 {} 个数据集", dataRefsMap.size());
                        } catch (Exception e) {
                            result.put("success", false);
                            result.put("error_hint", "data_refs 参数解析失败: " + e.getMessage());
                            return result;
                        }
                    } else if (dataRefId != null && !dataRefId.isEmpty()) {
                        // 单数据集模式（向后兼容）
                        DataEntry dataEntry = DATA_STORE.get(dataRefId);
                        if (dataEntry != null) {
                            dataJson = dataEntry.data;
                            log.info("[FixCode] 单数据集模式，data_ref_id={}", dataRefId);
                        }
                    }
                    ...
                    // 根据参数类型选择执行方法
                    if (dataRefsMap != null && !dataRefsMap.isEmpty()) {
                        // 多数据集模式
                        log.info("[FixCode] 使用多数据集执行，变量数: {}", dataRefsMap.size());
                        result = sandboxService.executeCodeWithMultipleDatasets(code, dataRefsMap, 30);
                    } else {
                        // 单数据集模式（向后兼容）
                        log.info("[FixCode] 使用单数据集执行");
